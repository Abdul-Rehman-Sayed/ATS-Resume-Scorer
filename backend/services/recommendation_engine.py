...
def generate_location_recommendations(location_results: Dict) -> List[Recommendation]:
    ...
    if privacy_risk == "high":
        ...
        impact = 2.0
        description = "Minor location information detected. Consider reviewing for unnecessary location details."
...
def generate_keyword_recommendations(
    ...
    if keyword_analysis:
        ...
        if missing:
            ...
            items = [
                f"Add '{kw}' to your resume in a relevant section" for kw in missing[:7]
            ]
            if len(missing) > 7:
        ...
        if gap:
            items = [
                f"Consider adding '{skill}' if you have this skill" for skill in gap[:5]
            ]
            if len(gap) > 5:
                items.append(
                    f"... and {len(gap) - 5} more skill(s) mentioned in the job"
                )
